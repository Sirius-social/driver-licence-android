...
class PoliceRequesterViewModel  @Inject constructor(
    ...
    fun loadingPoliceAnim() {
        ...
        policemanTextLiveData.postValue(String.format(App.getContext().getString(R.string.verify_police_policeman),item?.title?:""))
        Handler().postDelayed({
            policemanLoadingIconVisibilityLiveData.postValue(View.GONE)
            policemanSuccessIconVisibilityLiveData.postValue(View.VISIBLE)
            policemanTextLiveData.postValue(resourcesProvider.getString(R.string.verify_police_policeman_done))
            Handler().postDelayed({
